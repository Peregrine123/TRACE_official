...
import time
from safety.model.configuration_memgen import MemGenConfig
from safety.model.modeling_utils import (
...
class MemGenModel(PreTrainedModel, MemGenLoraSwitchMixin, MemGenGenerationMixin):
    ...
    def __init__(
        ...
        config: MemGenConfig, 
        base_tokenizer,
        reasoner: PreTrainedModel, 
        base_model_status: bool = False,
        sft: bool = False,
        ...
        # self.config = config
        if not sft:
            logging.info("close reasoner")
            fix_model_parameters(reasoner)
        
        fix_model_parameters(weaver_model)
        # fix_model_parameters(trigger_model)
        # self.peft_model = self._init_multi_adapter_model(
        #     base_model, 
        #     config.weaver_lora_config,
        #     reasoner_lora_config  # 新增reasoner的LoRA配置
        # )
        # self.reasoner = self.peft_model.base_model.model

        weaver_model = self._insert_lora_adapters(
            weaver_model, config.weaver_lora_config, 'weaver'
        )
        self.base_model_status = base_model_status
        self.sft = sft
        if base_model_status:
            self.reasoner = self._insert_lora_adapters(
            reasoner, config.weaver_lora_config, 'reasoner')
        else:
            self.reasoner = reasoner
        self.weaver = MemGenWeaver(weaver_model, config.prompt_latents_len, config.inference_latents_len, )
    ...
    def _forward(
        ...
        **kwargs
        ...
        for aug_point_idx in augmentation_indices:
            ...
            is_prompt_end_aug = (
                (labels[:, aug_point_idx] != -100).all().item()
                and
                (labels[:, aug_point_idx - 1] == -100).all().item()
            )
        ...
        if self.base_model_status:
            reasoner.set_adapter('reasoner')
    ...
    def forward(
        ...
        **kwargs
        ...
        # batch_size = input_ids.size(0) // 2  # Currently process one sequence per batch
        ...
        loss = self.chunked_cross_entropy(shift_logits, shift_labels, chunk_size=128)
    ...
    def _normalize_gate_subset_output(
        self,
        gate_out: Union[bool, List[bool], torch.Tensor],
        k: int,
        device: torch.device,
    ) -> torch.BoolTensor:
        """Normalize gate_fn output into BoolTensor[k]."""
        if isinstance(gate_out, bool):
            return torch.full((k,), gate_out, dtype=torch.bool, device=device)

        if isinstance(gate_out, (list, tuple)):
            if len(gate_out) != k:
                raise ValueError(f"gate_fn returned list length {len(gate_out)} != k {k}")
            return torch.tensor(gate_out, dtype=torch.bool, device=device)

        if torch.is_tensor(gate_out):
            x = gate_out.to(device=device, dtype=torch.bool)
            if x.dim() == 2 and x.size(1) == 1:
                x = x.squeeze(1)
            if x.shape != (k,):
                raise ValueError(f"gate_fn returned tensor shape {tuple(x.shape)} != ({k},)")
            return x

        raise TypeError(f"Unsupported gate_fn return type: {type(gate_out)}")
    ...
    def generate(
        ...
        gate_fn: Optional[Callable[[List[str]], Union[bool, List[bool], torch.Tensor]]] = None,
        # plain = False,
        **kwargs
        ...
        for i in range(max_new_tokens):
            ...
            # ===== Candidate selection + batch gate_fn (only for candidates) =====
            gate_mask = None
            if gate_fn is not None:
                if i == 0:
                    # prompt step: allow gating for all samples
                    candidate_mask = torch.ones((B,), dtype=torch.bool, device=device)
                else:
                    # inference: only delimiter-ending AND not over_limit are candidates
                    ends_with = self._check_ends_with_delimiter(
                        current_input_ids, tokenizer, self.delimiters
                    ).squeeze(1)  # [B] bool
                    over_limit = (sentence_augment_count >= max_augment_num)
                    candidate_mask = ends_with & (~over_limit)

                candidate_indices = candidate_mask.nonzero(as_tuple=True)[0]  # [K]
                gate_mask = torch.zeros((B,), dtype=torch.bool, device=device)

                if candidate_indices.numel() > 0:
                    # decode only candidates' generated parts
                    if current_input_ids.size(1) > prompt_len:
                        generated_ids = current_input_ids[candidate_indices, prompt_len:]
                        inference_texts = tokenizer.batch_decode(generated_ids, skip_special_tokens=True)
                    else:
                        inference_texts = [""] * candidate_indices.numel()

                    gate_out = gate_fn(inference_texts)  # bool / list[bool] / BoolTensor[K]
                    gate_sub = self._normalize_gate_subset_output(gate_out, candidate_indices.numel(), device)

                    gate_mask[candidate_indices] = gate_sub
            # ===================================================================
            augment_decision = self._should_augment(
                ...
                is_prompt=(i == 0),
                gate_triggered=gate_mask,  # BoolTensor[B] or None
            )
            ...
            if augment_indices.numel() > 0:
                ...
                merged_attention_mask = torch.zeros(
                    (B, new_len), device=device, dtype=current_attention_mask.dtype
                )
                ...
                if non_augment_indices.numel() > 0:
            ...
            # if all reached max augment num -> finish with reasoner.generate
            if (sentence_augment_count >= max_augment_num).all():
                rest_cfg = GenerationConfig(
                    do_sample=False,
    ...
    def from_config(cls, config_dict: dict):
        ...
        base_model_status = config_dict.get("base_model_status", False) 
        sft = config_dict.get("sft", False)
        # max augment numbers
        ...
        # if weaver_model_name and weaver_model_name != model_name:
        #     weaver_model = AutoModelForCausalLM.from_pretrained(weaver_model_name, torch_dtype=torch.bfloat16, attn_implementation="flash_attention_2")
        # else:
        #     weaver_model = base_model
        
        # if trigger_model_name and trigger_model_name != model_name:
        #     trigger_model = AutoModelForCausalLM.from_pretrained(trigger_model_name, torch_dtype=torch.bfloat16, attn_implementation="flash_attention_2")
        # else:
        ...
        return model
    def chunked_cross_entropy(self, shift_logits, shift_labels, chunk_size=128, ignore_index=-100):
        # shift_logits: [B, T, V], shift_labels: [B, T]
        B, T, V = shift_logits.shape
        loss_sum = shift_logits.new_zeros(())
        denom = shift_logits.new_zeros((), dtype=torch.long)

        for s in range(0, T, chunk_size):
            e = min(s + chunk_size, T)
            logits = shift_logits[:, s:e, :].contiguous().view(-1, V)
            labels = shift_labels[:, s:e].contiguous().view(-1)
        ...
        return loss_sum / denom.clamp_min(1).to(loss_sum.dtype)

    @torch.no_grad()
    def generate_without_weaver(
        self,
        input_ids: torch.Tensor,
        attention_mask: torch.Tensor,
        generation_config: GenerationConfig = None,
        ...
        **kwargs
        ...
        """
        Ablation: when augmentation is triggered, DO NOT use weaver's real output.
        Instead, replace the weaver latent hidden states with a random matrix of the same shape.
        This preserves sequence-length / mask behavior but destroys the semantic contribution of weaver.
        """
        ...
        for i in range(max_new_tokens):
            ...
            if gate_fn is not None:
                ...
                if candidate_indices.numel() > 0:
                    ...
                    gate_out = gate_fn(inference_texts)
                    gate_sub = self._normalize_gate_subset_output(gate_out, candidate_indices.numel(), device)
                    gate_mask[candidate_indices] = gate_sub
            # ===================================================================
            ...
            augment_decision = self._should_augment(
                ...
                temperature=generation_config.temperature,
                is_prompt=(i == 0),
                gate_triggered=gate_mask,
            )

            augmentation_pos[:, i] = augment_decision
            augment_indices = torch.where(augment_decision == 1)[0]

            # apply augmentation (RANDOM weaver hidden states)
            if augment_indices.numel() > 0:
                if i != 0:
                    sentence_augment_count[augment_indices] += 1
                ...
                # --- key ablation change: random latents instead of weaver output ---
                # We only need correct shape + attention mask length.
                latents_num = weaver.prompt_latents_num if i == 0 else weaver.inference_latents_num
                weaver_hidden_dim = weaver_inputs_embeds.size(-1)

                # random "weaver hidden states"
                weaver_hidden_states = torch.randn(
                    (augment_indices.numel(), latents_num, weaver_hidden_dim),
                    device=device,
                    dtype=weaver_inputs_embeds.dtype,
                )

                # attention mask for the appended latents (all valid tokens)
                attn_mask = torch.ones(
                    (augment_indices.numel(), latents_num),
                    device=device,
                    dtype=candidate_attention_mask.dtype,
                )
                # ---------------------------------------------------------------
